...
inpt = args.number
if inpt.find('0b') == 0:  # if 0b is the start of the string
    inpt = inpt[2:]  # strip off the 0b at the front
    all_binary = all(c in '01' for c in inpt)
    if all_binary:  # only 0 and 1 in the number
        print(' Binary Value: ', inpt)
        print('Decimal Value: ', int(inpt, 2))
        h = int(inpt, 2)
        print('    Hex Value: ', hex(int(h)))

    elif all_binary is False:
